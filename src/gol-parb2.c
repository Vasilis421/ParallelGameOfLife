...
world_border_wrap(world *world)
{
    ...
    // have been moved to world_timestep function
}
			   ...
			   double *comm, double * comp)
{
    ...
    double t1 = MPI_Wtime();
    // get the the neighbouring processes
    ...
    MPI_Isend(&cells[1][0], old->width + 2, MPI_INT, 
                  top_neighbour, 2, MPI_COMM_WORLD, &send_requests[0]);
    // recv first real row from bottom neighbour and set it as bottom ghost row
    MPI_Irecv(&cells[old->height + 1][0], old->width + 2, MPI_INT,
                  bottom_neighbour, 2, MPI_COMM_WORLD, &recv_requests[0]);
    // send last real row to bottom neighbour
    MPI_Isend(&cells[old->height][0], old->width + 2, MPI_INT,
                      bottom_neighbour, 2, MPI_COMM_WORLD, &send_requests[1]);
    // recv last real row from top neighbour and set it as top ghost row
    MPI_Irecv(&cells[0][0], old->width + 2, MPI_INT,
                      top_neighbour, 2, MPI_COMM_WORLD, &recv_requests[1]);
	
    double t2 = MPI_Wtime();
    // update inner board
    for (i = 2; i <= new->height - 1; i++) {
    ...
    // wait for recv's to complete before updating edge rows
    MPI_Waitall(2, recv_requests, MPI_STATUSES_IGNORE);
    double t4 = MPI_Wtime();
    
    // update edge rows
    for (j = 1; j <= new->width; j++) {
        new->cells[1][j] = world_cell_newstate(old, 1, j);
        new->cells[new->height][j] = world_cell_newstate(old, new->height, j);
    }
    double t5 = MPI_Wtime();
    
    // wait for send's to complete before moving to next iteration
    MPI_Waitall(2, send_requests, MPI_STATUSES_IGNORE);
    double t6 = MPI_Wtime();
    
    // add this step's computation and communication time to the total ones
    *comp += t3 - t2 + t5 - t4;
    *comm += t2 - t1 + t4 - t3 + t6 - t5;
}
                   ...
                   int* subworld_height) 
{
	...
	*subworld_height = height / p;
	// give 1 more row to some processes
	if (rank < remainder) { 
		*subworld_height += 1;
	}
    // adjust starting rows of all processes
	if (rank > remainder) { 
            ...
            int p, int rank, int start_row, int *heights, int *starts)
{
    ...
    if (rank == 0)
    {
        ...
        MPI_Scatterv(&whole_world->cells[1][0], heights,
                     starts, MPI_INT, &cur_world->cells[1][0],
                     heights[0], MPI_INT, 0, MPI_COMM_WORLD);
    // other processes
    } else {
        ...
        MPI_Scatterv(NULL, NULL, NULL, MPI_INT,
                     &cur_world->cells[1][0], 
                     subworld->height * (subworld->width + 2), 
...
main(int argc, char *argv[])
{
    ...
    // initialize random or fixed board in process 0 
    if (processor_rank == 0)
    {
        if (random_world) {
    ...
    // variables to measure computation and communication time
    double computation = 0;
    double communication = 0;
    // barrier so that all processes start at the same time
    ...
    for (n = 0; n < nsteps; n++) {
        ...
        world_timestep(cur_world, next_world, processor_rank,
		       number_of_processors, &communication, &computation);
    ...
    MPI_Reduce(&elapsed_time, &slowest_process_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
